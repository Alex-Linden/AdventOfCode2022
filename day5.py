...
for move in moves:
    ...
    from_col = int(move[3])
    to_col = int(move[-1])
    '''need to make the logic to account when num to move is greater than len of column'''
    if crates_dict_part_2[from_col]:
        """this is the old method need to be updated to [x:]"""
        """Use above logic to find x"""
        crate = crates_dict_part_2[from_col].pop()
        crates_dict_part_2[to_col].append(crate)
